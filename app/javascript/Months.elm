...
type alias Model = { months : Dict Int Month.Model }


type Msg = AddMonth (Month.Model, Cmd Month.Msg)
         | LoadEarlier
...
update msg model =
  case msg of
    LoadEarlier ->
        let
            newDate = Dict.keys model.months |> List.minimum |> Maybe.withDefault 1 |> Date.fromRataDie |> Date.add Date.Month -1
            newId = newDate |> Date.toRataDie
            (newMonth, newCmd) = Month.init newDate
        in
    ...
    AddMonth (monthModel, monthCmd) ->
        let
            newId = monthModel.date |> Date.toRataDie
        in
...
subscriptions model =
    Sub.batch <| List.map (\ (id, monthModel) ->
