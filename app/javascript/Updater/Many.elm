module Updater.Many exposing (Msg(Add, Delete, From, SendTo), Model, initModel, update, subscriptions)
...
type alias Model cModel cMsg =
    ...
    , viewAll :
        (Int
         -> cModel
         -> (Html cMsg -> Html (Msg cModel cMsg))
    ...
    converter
        ...
        , set =
            (\object model ->
                let
                    newObjects =
                        Dict.insert n object model.objects
                in
                    { model
                        | objects = newObjects
...
update msg model =
    case msg of
        Add ( objectModel, objectCmd ) ->
            let
                newObjects =
                    Dict.insert model.newID objectModel model.objects
            in
                { model
                    | objects = newObjects
        ...
        SendTo id cMsg ->
            model ! [ Cmd.map (objectC model.objUpdate id) <| toCmd cMsg ]
