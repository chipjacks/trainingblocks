...
RSpec.describe Activity, type: :model do
  ...
  describe "#run?" do
    ...
    it "returns false if no planned or completed laps are runs" do
      activity = Activity.from_strava_activity(build(:import, :laps))
      activity.data['laps'] = activity.data['laps'].map{ |l| l['type'] = Activity::OTHER }
  ...
  describe "#match_or_create" do
    ...
    it "creates a new activity when a match is found that has a different import" do
      ...
      activity.save!
      activity2 = Activity.from_strava_activity(create(:import, :laps, id: '12345', user: activity.user))
      activity2.id = '12345'
      expect {
        activity2.match_or_create
      }.to change{ Activity.all.length }
    ...
    it "updates an existing activity when a match is found" do
      ...
      activity2 = Activity.from_strava_activity(create(:import, :laps, user: activity.user))
      expect {
        activity2.match_or_create
      }.not_to change{ Activity.all.length }
      activity.reload
